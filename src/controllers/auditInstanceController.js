...
export const createAuditInstance = async (req, res) => {
  try {
    ...
    if (!plan) return res.status(400).json({ message: 'Invalid Audit Plan ID' });
    ...
    if (auditors.length !== assignedAuditors.length) {
      return res.status(400).json({ message: 'One or more assigned users are invalid or not auditors' });
    }
